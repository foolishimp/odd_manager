...
test('loadAllTickets reads tickets across all lanes', () => {
  const all = loadAllTickets(workspaceRoot);
...
test('rich-shape STDO ticket parses with mapped key set', () => {
  const surface = createTicketSurface(workspaceRoot);
  ...
  assert.ok(t.evaluationCriteria.length >= 6, 'T-007 has at least 6 evaluation criteria');
