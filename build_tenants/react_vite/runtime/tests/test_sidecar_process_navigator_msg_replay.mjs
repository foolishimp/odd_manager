      ...
      Object.freeze({
        ...
        detail: Object.freeze({
          ...
          cliTranscript: fixtureCliTranscript({
            ...
            label: 'transform.C/F_P worker terminal transcript',
            role: 'transform',
          ...
          cliTranscripts: Object.freeze([
            ...
            fixtureCliTranscript({
              ...
              label: 'evaluate.C/F_P design depth terminal transcript',
              role: 'evaluate',
              sourcePath: '/fixture/archive/design_depth_fp_evaluator_process_events.jsonl.trace/terminal.transcript',
              text: 'evaluation admitted',
            }),
          ]),
          stageProcesses: Object.freeze([
            Object.freeze({
              kind: 'sidecar_live_analysis_stage_process',
              id: 'stage-process:/fixture/archive/worker_process_started.json',
            ...
            Object.freeze({
              kind: 'sidecar_live_analysis_stage_process',
              id: 'stage-process:/fixture/archive/design_depth_fp_evaluator_process_started.json',
...
test('failed load clears stale process projection and process focus', async () => {
  ...
  const focused = module.updateSidecarState(loaded, { type: 'process/select-record', id: 'record-1' });
...
test('Msg-replay sequence: load to select-record produces deterministic final state', async () => {
  const module = await loadStateModule();
  const result = module.replaySidecarMessages(module.INITIAL_SIDECAR_STATE, [
    ...
    { type: 'process/select-record', id: 'record-1' },
  ]);
  ...
  assert.equal(result.state.ui.activeProcessRecordId, 'record-1');
  ...
  assert.equal(replayAgain.state.ui.activeProcessRecordId, result.state.ui.activeProcessRecordId);
