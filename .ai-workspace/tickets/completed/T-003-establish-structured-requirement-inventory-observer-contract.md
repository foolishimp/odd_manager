...
This ticket came from the `data_mapper.test32` observer-contract incident. The
current manager product now orients around observed `odd_sdlc` domain-pack
surfaces, generated requirement inventory, requirement-closure registers, and
domain UI pack boundaries in specification and runtime tests. Any remaining
observer gaps should be repriced into narrower tickets against the current
published contract rather than keeping this old incident ticket active.
