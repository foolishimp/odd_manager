...
This target depended on `T-022 TracedCalloutEvidence` inside the retired
odd_sdlc Process Navigator projection. Restoring that join would recreate a
removed carrier and conflict with the generic Project/run observation model.
T-031 now owns admitted ABG run, transcript, event, diagnostic, artifact, and
shell observation. Any future terminal/run correlation must re-enter against
that current contract rather than revive `TracedCalloutEvidence`.
