...
This ticket named the original Process View workbench buildout during the
control-surface reprice. The current product direction is now governed by the
entry-lens requirements, the current `ProcessWorkspace` implementation, and
more specific backlog/Sidecar/document-viewer tickets. Keeping this old broad
ticket active would duplicate current law rather than identify executable next
work.
