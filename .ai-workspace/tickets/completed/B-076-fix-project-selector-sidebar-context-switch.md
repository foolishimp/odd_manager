...
status: completed
review_status: accepted_then_superseded_by_t034
...
evaluation_criteria:
  ...
  - ticket, comment, session, file browser, Run Inspector, traversal, and document-surface reads use the selected Project root after the switch
...
At that review it remained active sprint work. Targeted replay, build, and focused browser proof
...
Sprint state at that review: implementation closure evidence was green, but
B-076 awaited operator transition.

## Prime-Set Acceptance 2026-07-11

The original context-switch fix is accepted from its recorded runtime, build,
browser, and live steel-thread evidence. T-034 subsequently removed the
Sidecar Project selector and made Build Portfolio activation plus shared
Project Context the canonical path. The old selector target is therefore
historical; current context-switch responsibility remains covered by T-034 and
the capability host.
