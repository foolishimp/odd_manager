...
superseded_by: current DocumentViewer browser-native PDF adapter design
dependencies:
  - B-066 completed
  - B-068 backlog
...
This ticket is completed by supersession, not by claiming its original
PDF.js/page-state closure law. The current ratified DocumentViewer design and
implementation provide one same-origin, Project-contained raw PDF route and
one browser-native inline PDF adapter, with Playwright proof over a real PDF
fixture and no base64 reducer payload.

Explicit page number, page count, per-page loading state, and a PDF.js worker
are no longer admitted current-product requirements. They require fresh intake
and design re-entry if the richer adapter is repriced later.
