...
The original broad rename ticket no longer represents a clean active work item.
The product now distinguishes Project from Workspace in `PRODUCT.md`, Sidecar
state and new work use `projectRoot`, and remaining `workspaceRoot` strings are
legacy compatibility/API residue that should be handled by narrower drift
tickets if they still matter. Keeping T-015 active would imply one large
tenant-wide sweep is still the governing work shape, which is no longer true.
