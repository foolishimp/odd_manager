...
This ticket named the original Requirements View workbench buildout during the
control-surface reprice. Its live product pressure is now carried by
`specification/requirements/10-entry-lenses-and-delivery-workspaces.md`, the
current `RequirementsWorkspace` implementation, and the active Sidecar
workbench tickets. Keeping this old broad ticket active would duplicate current
law and obscure the remaining concrete backlog.
